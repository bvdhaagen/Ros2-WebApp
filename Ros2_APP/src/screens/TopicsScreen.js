...
const TopicEchoScreen = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (rosInstance) {
        rosInstance.close();
        console.log('TopicEchoScreen: Cleaned up ROS connection');
      }
    };
  }, []);
  ...
  const subscribeToTopic = (topicName) => {
    ...
    try {
      // Try to determine message type (this is simplified)
      let messageType = 'std_msgs/String'; // Default
      ...
      topic.subscribe((message) => {
        ...
        if (messageType === 'sensor_msgs/PointCloud2') {
          ...
          displayData = `Linear: [${message.linear.x.toFixed(2)}, ${message.linear.y.toFixed(2)}, ${message.linear.z.toFixed(2)}], Angular: [${message.angular.x.toFixed(2)}, ${message.angular.y.toFixed(2)}, ${message.angular.z.toFixed(2)}]`;
  ...
  const reconnect = () => {
    if (ros) {
      ros.close();
      // Connection will be recreated by useEffect
